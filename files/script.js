...
function getFormattedNumber(num) {
    ...
    let n = Number(num);
    let value = n.toLocaleString("en"); //devolve a string com a representação do número

    return value;
}

function reverseNumberFormat(num) {
    return Number(num.replace(/,/g, '')); //assim coloca uma vírgula
}
...
for (let i = 0; i < operator.length; i++) {
    operator[i].addEventListener('click', function () {
        if (this.id == "clear") {
            //this.id vira o atual element id no HTML
            printHistory("");
            printOutput("");
